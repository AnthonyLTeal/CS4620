...
namespace CS4620IS
{
    ...
    public struct BoundingOrientedBox : IEquatable<BoundingOrientedBox>
    {
        ...
        public ContainmentType Intersects(ref Vector3[] triangle)
        {
            bool[] indicesContained = { false, false, false };
            for (int i = 0; i < 3; i++)
            {
                if (Contains(ref triangle[i]))
                    indicesContained[i] = true;
            }

            if (indicesContained[0] && indicesContained[1] && indicesContained[2])
                return ContainmentType.Contains;

            if (indicesContained[0] || indicesContained[1] || indicesContained[2])
                return ContainmentType.Intersects;

            float edgeLength1 = Vector3.Distance(triangle[1], triangle[0]);
            Ray ray1 = new Ray(triangle[0], Vector3.Normalize(triangle[1] - triangle[0]));
            float? ray1Dist = Intersects(ref ray1);
            if (ray1Dist.HasValue && ray1Dist.Value >= 0f && ray1Dist.Value <= edgeLength1)
                return ContainmentType.Intersects;

            float edgeLength2 = Vector3.Distance(triangle[2], triangle[1]);
            Ray ray2 = new Ray(triangle[1], Vector3.Normalize(triangle[2] - triangle[1]));
            float? ray2Dist = Intersects(ref ray2);
            if (ray2Dist.HasValue && ray2Dist.Value >= 0f && ray2Dist.Value <= edgeLength2)
                return ContainmentType.Intersects;
            
            float edgeLength3 = Vector3.Distance(triangle[0], triangle[2]);
            Ray ray3 = new Ray(triangle[2], Vector3.Normalize(triangle[0] - triangle[2]));
            float? ray3Dist = Intersects(ref ray3);
            if (ray3Dist.HasValue && ray3Dist.Value >= 0f && ray3Dist.Value <= edgeLength3)
                return ContainmentType.Intersects;
        ...
        public float? Intersects(ref Ray ray)
        {
            ...
            //Uncommenting this code might allow ray collision if the origin of the ray is in the box, not sure
            //chatgpt recommended it lol
            //if (t_min < 0.0f)
            //    return t_max;
